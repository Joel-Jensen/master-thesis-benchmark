...
time split transactions_1M.csv --verbose -n r/$cpus --filter='sudo -u postgres psql test -t -c "\\copy transactions (id, user_id, amount, type, country_code, platform, created_at) FROM STDIN"'
